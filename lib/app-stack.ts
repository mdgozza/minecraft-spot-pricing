...
import { Vpc, SecurityGroup, Peer, Port, InstanceType, InstanceClass, InstanceSize, SubnetType } from 'aws-cdk-lib/aws-ec2'
import {AsgCapacityProvider, Cluster, Compatibility, ContainerImage, Ec2Service, EcsOptimizedImage, NetworkMode, Protocol, TaskDefinition} from 'aws-cdk-lib/aws-ecs'
import {AutoScalingGroup, DefaultResult, LifecycleTransition} from 'aws-cdk-lib/aws-autoscaling'
  ...
  constructor(scope: Construct, id: string, props: Props) {
    ...
    const vpc = new Vpc(this, 'minecraft-vpc', {
      ...
      subnetConfiguration: [
        ...
        {cidrMask: 23, name: "Private", subnetType: SubnetType.PRIVATE_ISOLATED}
      ]
    })
    ...
    const asgCapacityProvider = new AsgCapacityProvider(this, 'minecraft-asg-capacity-provider', {
    ...
    taskDef.addVolume({efsVolumeConfiguration: {
      fileSystemId: fileSystem.fileSystemId},
      name: "minecraft"
    })

    const minecraftContainerDef = taskDef.addContainer("minecraft-container-def", {
      image: ContainerImage.fromRegistry(`itzg/minecraft-server:${props.minecraftImageTag}`),
    ...
    minecraftContainerDef.addMountPoints({containerPath: "/data", sourceVolume: "minecraft", readOnly: false})
